...
export function normalizeLead(body = {}) {
  ...
  const leadCode = str(body.leadCode || body.lead_code);
...
export async function submitLead(body, env = process.env) {
  ...
  // Nityavali lead creates can fail with "Duplicate value for leadCode" when the
  // tenant's auto-code generator collides. Always send a unique web lead code.
  if (!lead.leadCode) {
    lead.leadCode = `WEB-${Date.now().toString(36)}-${crypto.randomUUID().replace(/-/g, '').slice(0, 10)}`.toUpperCase();
  }
